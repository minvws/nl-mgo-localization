...
def make_organisation(data: dict[str, Any] | None = None) -> Organisation:  # type: ignore[explicit-any]
    data = data or {}
    return Organisation(
        id=data["id"] if "id" in data else faker.random_int(min=1),
        name=data["name"] if "name" in data else f"{faker.word()}.{faker.word()}@medmij".lower(),
...
def create_organisation(repo: OrganisationRepository, data: dict[str, Any] | None = None) -> Organisation:  # type: ignore[explicit-any]
    data = data or {}
