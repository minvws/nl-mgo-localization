...
def __get_address() -> list[dict[str, Any]]:  # type: ignore[explicit-any]
    return [
        {
            "city": "Utrecht",
            "extension": [
...
def __get_address_with_empty_extension() -> list[dict[str, Any]]:  # type: ignore[explicit-any]
    return [
        {
            "city": "Utrecht",
            "extension": [],
...
def __get_identifier() -> list[dict[str, Any]]:  # type: ignore[explicit-any]
    return [
        {
...
def __get_type() -> list[dict[str, Any]]:  # type: ignore[explicit-any]
    return [
        {
            "coding": [
                {
...
def __get_valueless_type() -> list[dict[str, Any]]:  # type: ignore[explicit-any]
    return [
        {
            "coding": [
                {
...
def test_hydrate_to_organization_missing_address(
    ...
    create_fhir_organization_without_address: FhirOrganization,
) -> None:
    hydration_service = HydrationService(
...
def test_hydrate_to_organization_uses_random_uuid_for_missing_identifier(
    ...
    addressing_mock_adapter = AddressingMockAdapter(
        ...
        endpoint_jwe_wrapper=mocker.Mock(),
    )
...
def test_hydrate_to_organization_handles_type_without_values(
    mocker: MockerFixture,
    create_fhir_organization_with_valueless_type: FhirOrganization,
    bindings: Generator[None, None, None],
) -> None:
    hydration_service = HydrationService(
        addressing_service=cast(AddressingService, AddressingMockAdapter()),
        logger=mocker.Mock(spec=Logger),
...
def test_hydrate_to_organization_with_empty_address_extension(mocker: MockerFixture) -> None:
    ...
    fhir_organization = FhirOrganization.model_validate(data)
    addressing_service = mocker.Mock(AddressingService)
    addressing_service.search_by_agb.return_value = ZalSearchResponseEntry(
    ...
    org = hydration_service.hydrate_to_organization(fhir_organization)
    assert isinstance(org, LoadOrganization)
    assert org.display_name == "Acme Corporation"
...
def test_get_organization_identifier_prefers_agb(mocker: MockerFixture) -> None:
    fhir_organization = FhirOrganization.model_validate(
        {
            "id": "f001",
            "identifier": [
                {"system": FHIR_NAMINGSYSTEM_URA, "value": "90000382"},
                {"system": FHIR_NAMINGSYSTEM_AGB_Z, "value": "71025100"},
            ],
            "active": True,
            "name": "Acme Corporation",
        }
    ...
    addressing_service = mocker.Mock(AddressingService)
    agb_entry = ZalSearchResponseEntry(
        medmij_id="agb@medmij",
        dataservices=[],
        organization_type="test_type",
        id_type="agb-z",
        id_value="71025100",
    )
    addressing_service.search_by_agb.return_value = agb_entry
    ...
    data_service_entry, identification = hydration_service._get_organization_identifier(fhir_organization)

    assert data_service_entry == agb_entry
    assert identification == "agb-z:71025100"
    addressing_service.search_by_ura.assert_not_called()


def test_get_organization_identifier_falls_back_to_ura(mocker: MockerFixture) -> None:
    fhir_organization = FhirOrganization.model_validate(
        {
            "id": "f001",
            "identifier": [
    ...
    addressing_service = mocker.Mock(AddressingService)
    addressing_service.search_by_agb.return_value = None
    ura_entry = ZalSearchResponseEntry(
        medmij_id="ura@medmij",
        dataservices=[],
        organization_type="test_type",
        id_type="ura",
        id_value="90000382",
    )
    addressing_service.search_by_ura.return_value = ura_entry
    ...
    assert data_service_entry == ura_entry
    assert identification == "ura:90000382"
    addressing_service.search_by_agb.assert_called_once_with("71025100")
    addressing_service.search_by_ura.assert_called_once_with("90000382")
